...
Содержателен режим 2. Экспертная БСД -- горизонтальная линия: её параметры
неверны, и данные ей не помогают. БСД с той же структурой и CPT, оценёнными
по данным, обгоняет экспертную версию начиная примерно с 1000 наблюдений и
дальше выходит на байесовский предел; деревьям и бустингу для сопоставимого
log-loss нужны тысячи наблюдений.

ВНИМАНИЕ, ЭТО ИЗМЕНИЛОСЬ ПОСЛЕ ПЕРЕСМОТРА СТРУКТУРЫ. Раньше обученной БСД
хватало 20 наблюдений, чтобы обойти экспертную. Теперь параметров 428
вместо 146, и на малых выборках оценивать их не из чего: при n = 20
обученная версия заметно ХУЖЕ экспертной. Более богатая структура лучше
описывает предметную область, но требует данных -- это её цена, и
замалчивать её не нужно.
...
которые вообще нужно оценивать (428 против произвольной функции пяти
