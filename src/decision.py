...
def evaluate(df: pd.DataFrame, actions: np.ndarray, costs: Costs,
    ...
    n = len(df)
    scale = per / n
    # ВНИМАНИЕ НА ЕДИНИЦЫ: первые четыре метрики нормированы на `per`
    # объектов (по умолчанию 100), а «средние_потери» -- на ОДИН объект.
    # Так сделано намеренно: счётные величины читаются как «столько-то
    # выездов на сотню», а потери сравниваются между стратегиями, и делить
    # их на сотню значило бы только добавить нулей. Но в одной таблице
    # соседствуют разные единицы, и это надо помнить.
    return {
        ...
        "средние_потери": float(L[a_idx, s_idx].mean()),   # на один объект
    }
