...
if __name__ == "__main__":
    ...
    print(f"""
ОГОВОРКА О БАЗОВОЙ ЧАСТОТЕ. В синтетическом парке P(срочное) = {base_rate:.1%},
потому что априорные вероятности входных узлов (network_spec.PRIOR) -- это
допущение для симуляции, а не измерение. На пилоте 32 объектов срочных
случаев было порядка одного в месяц, то есть база примерно вчетверо ниже.
Поэтому абсолютные значения «выездов на 100 объектов» выше реальных;
содержательны здесь сравнения стратегий между собой при одинаковой базе,
а не сами уровни. Как только появится анонимизированная выгрузка из АИС,
PRIOR заменяется частотами из неё, и уровни становятся сопоставимы с пилотом.""")
